...
const FarmCards: React.FC = () => {
  ...
  const okfIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'OKF',
  )

  const okfPrice =
    okfIndex >= 0 && stakedValue[okfIndex]
      ? stakedValue[okfIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2372500)
  const OKF_PER_BLOCK = new BigNumber(60)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [okf, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'OKF' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
